# Azazel-Covenant (formerly Azazel-Common): Migration Plan
...
before either Edge or Grimoire integrated the package at all. `v0.3.0`
renamed the package itself (repository, distribution, and import
namespace) from `Azazel-Common`/`azazel_common` to
`Azazel-Covenant`/`azazel_covenant`; see the note under Phase 4 for what
...
| 2 | Introduce into Azazel-Grimoire | **Not started** — gated on a dependency-policy exception; Azazel-Grimoire's core dependency set (stdlib + PyYAML + idna + PyNaCl) excludes `pydantic` today, so adoption needs a `pyproject.toml` change plus an ADR and owner decision on the Grimoire side before any code lands |
| 3 | Introduce into Azazel-Edge | **Plan-doc stage only** — `docs/AZAZEL_COMMON_EDGE_ADAPTER_PLAN.md` in the `Azazel-Edge` repository (dated 2026-07-09, the Issue 5 deliverable, filename not yet updated for the rename) proposes emit-alongside adapters for `DecisionExplanation`/`TrustCapsule`/`AuditEvent`; no code, no dependency pin yet. That plan explicitly defers real Edge↔Grimoire integration (and therefore real `cti_contracts` usage on the Edge side) to FY2027+ |
| 4 | Introduce into Azazel-Gadget | **Effectively done, ahead of order, migration to `v0.3.0` pending** — Azazel-Gadget currently pins `azazel-common @ git+...@v0.2.0` (the old distribution name) in `requirements.txt`, emits `StatusView` alongside its own snapshot (`py/azazel_gadget/common_view.py`), reads it back (`control_plane.py`), and surfaces it via its web API (`/api/state`, `status_view` key) using the `product_view={"gadget_snapshot": ...}` superset pattern. Note this happened via the `view` module (`v0.2.0`), which was not part of this phase's original scope (`StateSnapshot`/`ModeState`/`ActionIntent`/`AuditEvent`/notify) — the phase's *intent* (Gadget as a real Covenant consumer) is satisfied, but not via the exact schema list originally planned, and it landed before Phases 2/3. Gadget's migration to the `v0.3.0` `azazel-covenant`/`azazel_covenant` names is a follow-up, not yet done |
...
or rewriting a working code path in Edge, Gadget, or Grimoire. No phase
moves Edge's Deterministic Arbiter, Edge's NOC/SOC Evaluators, Grimoire's
...
## Phase 1 — Bootstrap `Azazel-Covenant` package, schema-only
...
Add the `src/azazel_covenant` package to this repository per
`repository-layout.md`. First release is `v0.1.0` and contains **only**:

- `azazel_covenant.schema` (`StateSnapshot`, `ModeState`, `ActionIntent`,
  `EvidenceRef`, `DecisionExplanation`, `AuditEvent`, `TrustCapsule`)
- `azazel_covenant.cti_contracts` (`CtiEventBatch`, `CtiFlowBatch`,
...
Exit condition: `v0.1.0` tagged. Nothing outside `Azazel-Covenant` depends on
it yet.

### Rename note (not a phase — a `v0.3.0` housekeeping event)

`v0.3.0` renamed the repository from `01rabbit/Azazel-Common` to
`01rabbit/Azazel-Covenant`, the distribution from `azazel-common` to
`azazel-covenant`, and the import namespace from `azazel_common` to
`azazel_covenant`. This is a breaking change to every consumer's import
...
## Phase 2 — Introduce into Azazel-Grimoire

**Status: Not started.** Blocked on a dependency-policy exception on the
Grimoire side (see status table above) — this phase's description below is
still the intended approach once that gate clears.

Azazel-Grimoire adopts `azazel_covenant.cti_contracts` for payload
...
Covenant schema, running **alongside** (not replacing) existing validation
...
Exit condition: Grimoire's four endpoints validate against Covenant schemas
in CI; no observed behavior change for existing callers; existing Grimoire
...
Edge↔Grimoire integration to FY2027+. The description below is that plan's
target, not yet implemented.

Edge adopts Covenant schemas for:

- the payloads it sends to Grimoire (`CtiEventBatch`/`CtiFlowBatch`/
...
gets sent to Grimoire, and what shape decisions/audit records are
serialized into — changes. The arbiter still decides; Covenant only
...
validate against Covenant schemas in CI; existing demo flows (including
...
(`azazel-covenant`/`azazel_covenant`) is a small follow-up in the Gadget
repository, not yet done.

Gadget adopts Covenant schemas for `StateSnapshot`, `ModeState`,
...
reviewed as *input* to `azazel_covenant.paths` design (Phase 5), not
replaced in this phase.

Exit condition: Gadget's state/mode/audit/notification payloads validate
against Covenant schemas in CI; Gadget's existing hardware-control code
paths untouched.
...
- `azazel_covenant.paths` (informed by Gadget's existing path-schema and
...
- `azazel_covenant.api` (token/role/fail-closed helpers)
- `azazel_covenant.notify` (ntfy/Mattermost/SSE payload helpers)
...
at least two of {Edge, Gadget, Grimoire} in favor of the Covenant helper,
...
`Azazel-Boot` and any new Azazel-series tool depend on `Azazel-Covenant`
...
- SemVer. Proposed progression:
  ...
  - `v0.3.0` — repository/distribution/import namespace renamed to
    Azazel-Covenant/`azazel_covenant` (breaking; no schema change)
  - `v0.4.0` — api auth helper + notify model added
  - `v0.5.0` — CTI contract stabilized against real Edge↔Grimoire traffic
  - `v1.0.0` — after Edge, Gadget, and Grimoire each pass their contract
    test suite against Covenant
- Consumers pin an exact tag via
  `azazel-covenant @ git+https://github.com/01rabbit/Azazel-Covenant.git@vX.Y.Z`
...
- Any breaking schema change is a major version bump, ships with a
  migration note in `Azazel-Covenant`'s own `CHANGELOG.md`, and is adopted
  by consumers on their own schedule — never forced by a Covenant release
...
`Azazel-Covenant` itself, and does not touch any other phase.
