# Azazel-Fabric (formerly Azazel-Common): Issue Breakdown
...
## Issue 1 — Design Azazel-Fabric (this task)

**Repository:** `Azazel-Fabric` (design record, this repository).
...
## Issue 2 — Bootstrap Azazel-Fabric v0.1.0, schema-only

**Repository:** `Azazel-Fabric` (this repository).
**Goal:** Add the `src/azazel_fabric` package per
`repository-layout.md` and ship
`azazel_fabric.schema` + `azazel_fabric.cti_contracts` only, per
...
## Issue 3 — Define CTI advisory contracts for Edge-Knowledge integration

**Repository:** `Azazel-Fabric` (schema definition) with review input from
`Azazel-Knowledge` and `Azazel-Edge` maintainers.
**Goal:** Finalize `CtiContextRequest`/`CtiContextResponse` (and the three
ingest batch shapes) against Azazel-Knowledge's actual current API, per
`contracts.md` §2.
**Deliverables:** finalized field lists, explicit confirmation that
`behavioral_cti` omission-when-absent semantics match Knowledge's existing
behavior, explicit confirmation no directive-shaped field exists in the
response.
**Non-goals:** no change to Azazel-Knowledge's running API in this issue —
schema-definition only.
**Acceptance:** schema reviewed against real Knowledge request/response
samples (recorded as fixtures in `azazel_fabric.testing.contract_cases`);
advisory-only invariant confirmed by both Edge and Knowledge maintainers.

## Issue 4 — Add contract tests for CTI context and reaction payloads

**Repository:** `Azazel-Fabric` (test authorship), exercised against
`Azazel-Knowledge` in CI.
**Goal:** Catch payload drift between what Edge/Gadget send and what
Knowledge accepts, and between what Knowledge returns and what Edge/Gadget
expect.
**Deliverables:** `azazel_fabric.testing.contract_cases` populated with
canonical valid/invalid payloads; a CI job (in `Azazel-Knowledge`, consuming
Fabric) that validates live endpoint behavior against these cases.
**Non-goals:** does not change Knowledge's existing validation logic — sits
alongside it.
**Acceptance:** CI job passes against current Knowledge `main`; a
...
real Edge↔Knowledge integration to FY2027+; no adapter code has landed yet.
...
Explanation and Audit Logger output through `azazel_fabric.schema`, and
for sending CTI payloads through `azazel_fabric.cti_contracts`, per
`migration-plan.md` Phase 3.
**Deliverables:** a short design note identifying exact call sites in
Edge's existing Decision Logger / Knowledge client code that would wrap
Fabric schemas, with an explicit statement of which files/functions are
...
**Repository:** `Azazel-Fabric`, informed by `Azazel-Gadget`'s existing
path/legacy-migration design.
**Goal:** `azazel_fabric.paths` design and implementation, per
...
migration output for representative Edge/Gadget/Knowledge path layouts; no
destructive operation possible without explicit non-dry-run opt-in by the
calling product.

## Issue 8 — Define shared notification event model

**Repository:** `Azazel-Fabric`.
**Goal:** `azazel_fabric.notify` design and implementation, per
`migration-plan.md` Phase 5.
...
operate inside the new `Azazel-Fabric` repository, on largely independent
...
otherwise independent of the Edge/Gadget/Knowledge integration issues — they can
