"""Azazel-Covenant (formerly Azazel-Common): shared contracts for the Azazel series.
...
``v0.1.0`` shipped schema-only: :mod:`azazel_covenant.schema` and
:mod:`azazel_covenant.cti_contracts`. ``v0.2.0`` adds :mod:`azazel_covenant.view`,
...
``azazel-covenant``/``azazel_covenant`` (breaking; see ``CHANGELOG.md``). The
``paths``/``audit``/``api``/``notify`` helpers are later phases and are
intentionally not present yet.
"""

from azazel_covenant.version import __version__
